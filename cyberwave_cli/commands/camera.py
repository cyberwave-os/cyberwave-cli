...
def camera(
    ...
    Examples:
        # Local webcam
        cyberwave camera
        cyberwave camera -c 0 -f 15

        # IP camera with authentication
        cyberwave camera -u rtsp://192.168.1.100:554/stream --camera-user admin --camera-pass secret

        # Use local edge code (development)
        cyberwave camera --local-edge ./cyberwave-edge-python --env-only

        # Non-interactive with existing twin
        cyberwave camera -t TWIN_UUID -e ENV_UUID -y

        # Named environment
        cyberwave camera -n "Warehouse Cameras"
    """
    ...
    if not creds or not creds.token:
        ...
        console.print("[dim]Run [bold]cyberwave login[/bold] or [bold]cyberwave configure --token YOUR_TOKEN[/bold][/dim]")
        raise click.Abort()
